...
[{include file="email/html/header.tpl" title="DD_ORDERPROCESSED_HEADING"|oxmultilangassign|cat:" #"|cat:$order->oxorder__oxordernr->value style=$style}]

    [{block name="email_html_ordershipped_sendemail"}]
       [{oxcontent ident="oxorderprocessedemail"}]
    [{/block}]
        ...
        <tbody>
            [{block name="email_html_ordershipped_orderarticles"}]
                [{foreach from=$order->getOrderArticles(true) item=oOrderArticle}]
                    <tr valign="top">
                        <td align="right" class="text-center">[{$oOrderArticle->oxorderarticles__oxamount->value}]</td>
                        <td>
